...
class CSFramework extends CSFramework_Abstract {
  ...
  public function validate_save( $request ) {
    ...
    if ( isset( $request['import'] ) && ! empty( $request['import'] ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Success. Imported backup options.','mevo' ), 'updated' );
    }

    // reset all options
    if ( isset( $request['resetall'] ) ) {
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored.','mevo' ), 'updated' );
      return;
    }
    ...
    if ( isset( $request['reset'] ) && ! empty( $section_id ) ) {
      ...
      $add_errors[] = $this->add_settings_error( esc_html__( 'Default options restored for only this section.','mevo' ), 'updated' );
    }
  ...
  public function admin_menu() {
    ...
    if( $args['menu_type'] == 'adds_submenu_page' ) {
  ...
  public function admin_page() {
    ...
    print '<div class="cs-framework cs-option-framework">';
      ...
      print '<fieldset>';
      print ( $this->settings['ajax_save'] === true ) ? '<span id="cs-save-ajax">'. esc_html__( 'Settings saved.','mevo' ) .'</span>' : '';
      submit_button( esc_html__( 'Save','mevo' ), 'primary', 'save', false, array( 'data-ajax' => $this->settings['ajax_save'], 'data-save' => esc_html__( 'Saving...','mevo' ) ) );
      submit_button( esc_html__( 'Restore','mevo' ), 'secondary cs-restore cs-reset-confirm', $this->unique .'[reset]', false );
      print '</fieldset>';
      print ( empty( $has_nav ) ) ? '<a href="#" class="cs-expand-all"><i class="fa fa-eye-slash"></i> '. esc_html__( 'show all options','mevo' ) .'</a>' : '';
